...
public class SsoController {
    ...
    @GetMapping(value = {"index.html","index.html/{userId}"})
    public String index(@PathVariable(required = false) Long userId, ModelMap modelMap){
        if (userId==null){
            modelMap.put("userId", "");
        }else {
    ...
    public String checkLogin(User user , HttpServletRequest request, HttpServletResponse response, ModelMap modelMap){
        ...
        if (ResultBeanConstant.OK.equals(resultBean.getStatusCode())){
            ...
            if (isAdmin!=null){
                logger.info("管理员[{}]登录成功！",user.getId());
        ...
        //登录失败，跳转到登录界面
        logger.error("用户[{}]登录失败！",user.getId());
